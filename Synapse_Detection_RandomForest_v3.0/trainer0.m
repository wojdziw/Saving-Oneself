...
function [FeatureList] = trainer0(layer,truthfile,imagefile)
%trainer0 A function to generate Not Synapse data for use in TreeBagger
%   Finds and windows lots of objects which we know from truth files are
%   not synapses but my filters have extracted

%Initialise as empty to prevent errors
FeatureList=[];
%Get the current predicted synapse using my method
...
%Only include my prediction which are definitely false
ImSynapse = im2bw(im2bw(truthfile(:,:,layer)).*CurrentGen);

%Get centroids
ImNotSynapse = im2bw(CurrentGen.*(~ImSynapse)); 
tcent = regionprops(ImNotSynapse,'centroid');
...
for i= 1:WindowNum
TruthWindow = imcrop(ImNotSynapse,[Locationsrange(i,1),Locationsrange(i,2),winsize,winsize]);
...
%Save both incase of later usage
...
%Also make sure traner1 and Tester are exactly the same!!!
